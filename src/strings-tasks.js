...
function removeLastOccurrences(str, value) {
  return str.replace(new RegExp(`(${value})\\b`, 'g'), '');
}
...
function countVowels(str) {
  ...
  for (let i = 0; i < str.split('').length; i += 1) {
    if (
      ['a', 'e', 'i', 'o', 'u', 'y'].includes(str.split('')[i].toLowerCase())
    ) {
      counter += 1;
    }
  }
...
function isPalindrome(string) {
  ...
  const s = string.toLowerCase().replace(/[^a-zа-яё]+/g, '');
...
function findLongestWord(str) {
  ...
  const foundWords = words.filter((word) => word.length === longestWordLength);
  return foundWords[0];
}
/**
...
function invertCase(str) {
  ...
  // .split('')
  // .map((el, ) =>
  //   (el = el.toUpperCase() !== el ? el.toLowerCase() : el.toUpperCase())
  //   .join('')
...
function extractNameFromTemplate(str) {
  return str.split(',')[1].trim().slice(0, -1);
}
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((el) => ROT[VOCABULARY.indexOf(el)])
    .join('');
}
